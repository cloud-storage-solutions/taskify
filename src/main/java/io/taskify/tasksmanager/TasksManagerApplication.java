...
public class TasksManagerApplication {
	...
	CommandLineRunner initializeRepository(final TasksRepository tasksRepository) {
		...
		Task task4 = Task.builder().name("task-4").description("potki3").isCompleted(false)
				.dueTo(LocalDate.of(2012, Month.AUGUST, 3)).assignedUser(lubo).build();
		Task task5 = Task.builder().name("task-4").description("potki4").isCompleted(false)
				.dueTo(LocalDate.of(2012, Month.AUGUST, 3)).assignedUser(lubo).build();
		...
		return args -> {
			...
			System.out.println("********\nLubo's tasks = " + tasksRepository.findAllUnfinishedTasksPerUser(lubo)
					+ "\n************");
		};
	}
